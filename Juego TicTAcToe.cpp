...
char jugadorActual = 'O';//Variable con que se inicia el juego, esta puede cambiar en el transcurso
...
void presentacion(){
	...
	cout << "\n\t\t\t ====================================================================";//Decoracion
	cout << "\n\t\t\t   Programa elaborado por:@ Andres Isai Vasquez Vasquez (VV18009) \n";
	cout << "\n\t\t\t                  TIC TAC TOE GAME                                 \n";
    cout << "\n\t\t\t ====================================================================";//Decoracion
    cout << "\n\t\t\t =                            1. JUGAR                              =";//Caso 1
...
void juego(){
	...
	while(!hayGanador() && !hayEmpate()){
		...
		if(tablero[fila][columna] != '_'){//Condicional si para que cuando una casilla este vacia
			...
			if(jugadorActual == 'O'){
				jugadorActual = 'X';//Si el jugador X no esta jugando es O que lo hace
			}
			else{
				jugadorActual = 'O';//Sino es X que esta jugando
			}
		}
		cout << "\n";
		system("pause");//Pausa de sistema
